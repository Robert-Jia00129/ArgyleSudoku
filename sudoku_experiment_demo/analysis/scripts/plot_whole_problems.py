...
def plot_comparison_for_constraint(files_directory, constraint_true='distinct', constraint_false='PbEq',
    ...
    plt.grid(True)
    if save_as_file:
        os.makedirs(Path(save_as_file).parent,exist_ok=True) # make sure the directory exist
...
if __name__ == '__main__':
    ...
    #                                    save_as_file=f'../images/whole_{true_constraint}_{time_cap}s_cap', time_cap=time_cap)

    plot_comparison_for_constraint(files_directory,"distinct","PbEq", save_as_file=f'../images/whole_distinct_{time_cap}s_cap', time_cap=time_cap)
