...
class ExperimentTimeRecordingError(Warning):
    pass


class ExperimentResultRecordingError(Warning):
    pass
...
class ConstraintPlotter:
    ...
    def set_graph_properties(self, x_max=5, y_max=5, width=10, height=5, opacity=0.6, marker_size=None,
        ...
        self.x_true_max = x_max
        self.y_false_max = y_max
        self.width = width
        self.height = height
    ...
    def plot_constraints_comparison(self, constraint_comparison_num, solvers=None, combined_plot=False, constraint_names=None):
                    ...
                    if complement_key in entry:
                        ...
                        for solver_key in true_data:
                            if solver_key != 'smt_path' and (solvers is None or solver_key in solvers):
                                # true_data[solver_key]: (time, did_timeout, "sat"/"unsat"/"unknwon")
                                true_did_timeout = true_data[solver_key][1]
                                false_did_timeout = false_data[solver_key][1]

                                true_answer = true_data[solver_key][2]
                                false_answer = false_data[solver_key][2]

                                # check for answer consistency
                                if true_answer != "unkown" and false_answer != "unknown":
                                    if true_answer != false_answer:
                                        warnings.warn("The answer when condition is set to true and condition is set to false are different: \n"
                                                      f"When condition is set to true, the answer is {true_answer}\n"
                                                      f"When condition is set to false, the answer is {false_answer}\n"
                                                      f"This occured for the entry: {entry}", ExperimentResultRecordingError)

                                    if ans is None:
                                        ans = true_answer
                                    if true_answer != ans: # check with other answers for this problem
                                        warnings.warn(f"The results for the problem {entry} are not consistent across solvers or conditions")

                                time_true = self.x_true_max if true_did_timeout else true_data[solver_key][0]
                                time_false = self.y_false_max if false_did_timeout else false_data[solver_key][0]
        ...
        for i, solver_key in enumerate(solvers):
            if solver_key in times_constraint_true and solver_key in times_constraint_false:
                if len(times_constraint_true[solver_key]) != len(times_constraint_false[solver_key]):
                    warnings.warn("The experiment contains unbalanced time when the condition is set to true and condition is set to false\n"
                                                                                                         "This might lead to invalid comparison between instances, attempting to continue using the minimum length", ExperimentTimeRecordingError)
                    ...
                    true_times = times_constraint_true[solver_key]
                    false_times = times_constraint_false[solver_key]
                # adjusting the datapoints to be within or at the edge of the graph
                true_times = np.clip(true_times, 0, self.x_true_max)
                false_times = np.clip(false_times, 0, self.y_false_max)
                for ele in false_times:
                    if ele > self.y_false_max:
                        raise "AS"

                axs[i].scatter(true_times, false_times, alpha=self.opacity,s=self.marker_size)
                axs[i].plot([0, self.x_true_max], [0, self.y_false_max], self.line_style)
                ...
                axs[i].set_xlim([0, self.x_true_max])
                axs[i].set_ylim([0, self.y_false_max])
        ...
        if combined_plot:
            ...
            axs[-1].plot([0, self.x_true_max], [0, self.y_false_max], 'r--')
            ...
            axs[-1].set_xlim([0, self.x_true_max])
            axs[-1].set_ylim([0, self.y_false_max])
...
if __name__ == '__main__':
    ...
    warnings.filterwarnings("once", category=ExperimentTimeRecordingError)
    warnings.filterwarnings("once", category=ExperimentResultRecordingError)
    ...
    plotter.plot_constraints_comparison(1, solvers=["z3"],  combined_plot=False, constraint_names=constraint_names)
