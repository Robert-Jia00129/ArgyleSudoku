...
def plot_constraints_comparison(constraint_comparison_num, solver="ALL", file_path=''):
    ...
    for entry in parsed_data:
        for constraint, solvers in entry.items():
            if type(constraint) == tuple:
                constraint_value = constraint[constraint_comparison_num]

                for solver_key, values in solvers.items():
                    if solver_key != 'smt_path':  # Exclude the 'smt_path' key
                        time = values[0][0]  # Extract the time value

                        # Check if the current solver matches the specified solver or if all solvers are requested
                        if solver == "ALL" or solver == solver_key:
    ...
    # Plotting the data
    num_plots = len(times_constraint_true) + 1 if solver == "ALL" else 1
